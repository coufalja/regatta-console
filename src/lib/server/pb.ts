...
export interface Value {
	[index: string]: number | string | boolean | Struct | ListValue | undefined;
...
const encoders = {
	[typeOf({})]: (v: JsonObject) => wrap(Kind.Struct, struct.encode(v)),
	[typeOf([])]: (v: JsonArray) => wrap(Kind.List, list.encode(v)),
	[typeOf(0)]: (v: number) => wrap(Kind.Number, v),
	[typeOf('')]: (v: string) => wrap(Kind.String, v),
	[typeOf(true)]: (v: boolean) => wrap(Kind.Bool, v),
...
function wrap(kind: Kind, value: string | number | boolean | Struct | ListValue): Value {
...
export const value = {
	...
	encode(value: JsonValue): Value {
		...
		// @ts-expect-error bad type checking.
...
export const struct = {
	 ...
	 *
	 * @param {Object.<string, *>} json the JSON object.
	 * @returns {Struct}
	 */
